...
export class RecommendationsController {
    ...
    getRecommendedFeed(@Req() req: any, @Query() query: GetRecommendationFeedQueryDto) {
    ...
    debugClassifyCaption(@Body() dto: DebugClassifyCaptionDto) {
        const debugEnabled = this.configService.get<string>('RECOMMENDATION_DEBUG_ENABLED') === 'true';
        const isProduction = this.configService.get<string>('NODE_ENV') === 'production';

        if (isProduction && !debugEnabled) {
            throw new ForbiddenException('Recommendation debug classification is disabled in production');
        }
