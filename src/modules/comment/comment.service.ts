...
export class CommentService {
  ...
  constructor(
    ...
    @InjectModel(CommentLog.name)
    private readonly commentLogModel: Model<CommentLogDocument>,
  ...
  async create(createCommentDto: CreateCommentDto, userId: string) {
    ...
    const [post] = await this.eventEmitter.emitAsync(
      AppEvents.POST_GET_USER_ID,
      ...
      uniqueTaggedIds.forEach(async (taggedId) => {
        if (taggedId.toString() !== userId) {
          const canView = await this.eventEmitter.emitAsync(
            ...
            { postId: postId.toString(), viewerId: taggedId.toString() },
          );
  ...
  private async trackRecommendationInteraction(
    ...
    try {
      ...
      this.logger.warn(
        `Không thể ghi nhận recommendation interaction cho comment: ${error.message}`,
      );
    }
  }
  ...
  async remove(commentId: string, userId: string) {
    ...
    await this.commentLogModel.deleteMany({
      commentId: new Types.ObjectId(commentId),
    });
  ...
  async findByPostId(postId: string, viewerId?: string) {
    ...
    comments = comments.map((comment: any) => {
      ...
      if (
        ...
        typeof comment.parentId.userId === 'object' &&
        comment.parentId.userId._id
      ) {
        comment.parentId.userId = {
