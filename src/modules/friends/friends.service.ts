...
export class FriendsService {
  ...
  async sendFriendRequest(
    ...
    const existingFriendship = await this.friendModel.findOne({
      $or: [
        {
          user_id: new Types.ObjectId(senderId),
          friend_id: new Types.ObjectId(receiver_id),
        },
        {
          user_id: new Types.ObjectId(receiver_id),
          friend_id: new Types.ObjectId(senderId),
        },
      ],
    });
    ...
    if (existingFriendship) {
      throw new HttpException(
        'Bạn đã là bạn bè với người này',
        HttpStatus.CONFLICT,
      );
    }
    ...
    const existingRequest = await this.friendRequestModel.findOne({
      $or: [
        ...
        {
          sender_id: new Types.ObjectId(receiver_id),
  ...
  async removeFriend(userId: string, removeFriendDto: RemoveFriendDto) {
    ...
    if (!friendship) {
      throw new HttpException(
        'Bạn không phải bạn bè với người này',
        HttpStatus.NOT_FOUND,
      );
    }
    ...
    await this.friendModel.deleteMany({
      $or: [
        ...
        {
          user_id: new Types.ObjectId(friend_id),
          friend_id: new Types.ObjectId(userId),
        },
      ],
    });
    ...
    await this.friendRequestModel.deleteMany({
      $or: [
        ...
        {
          sender_id: new Types.ObjectId(friend_id),
  ...
  @OnEvent(AppEvents.FRIEND_IDS_GET)
  async onGetFriendIdsEvent(payload: { userId: string }): Promise<string[]> {
    if (!Types.ObjectId.isValid(payload.userId)) {
      return [];
    }

    const friendIds = await this.friendModel
      .distinct('friend_id', { user_id: new Types.ObjectId(payload.userId) })
      .exec();

    return friendIds.map((friendId) => friendId.toString());
  }
  ...
  async onCheckFriendRelationship(payload: {
    userId: string;
    targetUserId: string;
  }) {
  ...
  async getRelationshipStatus(userId: string, targetUserId: string) {
    ...
    if (receivedRequest) {
      return {
        ...
        message: 'Có lời mời kết bạn chờ phản hồi',
        requestId: receivedRequest._id,
      }; //, requestId: receivedRequest
    }
  ...
  async getFriendSuggestions(
    userId: string,
    suggestionsDto: FriendSuggestionsDto,
  ) {
    const { limit = 10, page = 1 } = suggestionsDto;
    const skip = (page - 1) * limit;

    // Lấy thông tin user hiện tại
    const [userResult] = await this.eventEmitter.emitAsync(
    ...
    const [friends, sentRequests, receivedRequests] = await Promise.all([
      ...
      this.friendRequestModel
        .find({ sender_id: new Types.ObjectId(userId) })
        .select('receiver_id'),
      this.friendRequestModel
    ...
    const currentUserBirthYear = currentUserDateOfBirth
      ? parseInt(currentUserDateOfBirth.substring(0, 4)) || 0
      : 0;
    ...
    const pipeline: PipelineStage[] = [
      ...
      // Tính điểm cơ bản dựa trên bạn chung và yếu tố ngẫu nhiên
      {
        $addFields: {
          mutualFriendsCount: {
            $ifNull: [{ $arrayElemAt: ['$mutualFriends.mutualCount', 0] }, 0],
          },
          randomScore: { $rand: {} },
            ...
            $add: [
              {
                $multiply: [
                  {
                    $ifNull: [
                      { $arrayElemAt: ['$mutualFriends.mutualCount', 0] },
                      0,
                    ],
                  },
                  1000,
    ...
    const [suggestionsRawResult, countResult] = await Promise.all([
      this.eventEmitter.emitAsync(AppEvents.USER_AGGREGATE, {
    ...
    const coAppearanceScores =
      await this.faceCoAppearanceService.getCoAppearanceScores(userId);
    ...
    const suggestions = suggestionsRaw.map((user: any) => {
      ...
      if (
        user.currentCity &&
        currentUserCurrentCity &&
        user.currentCity === currentUserCurrentCity
      ) {
        score += 150;
      ...
      if (
        user.hometown &&
        currentUserHometown &&
        user.hometown === currentUserHometown
      ) {
        score += 150;
      ...
      if (
        ...
        user.workplace === currentUserWorkplace
      ) {
        score += 150;
      ...
      if (
        ...
        currentUserRelationshipStatus &&
        user.relationshipStatus === currentUserRelationshipStatus
      ) {
        score += 100;
      ...
      if (user.dateOfBirth && currentUserBirthYear > 0) {
        ...
        if (
          userBirthYear > 0 &&
          Math.abs(userBirthYear - currentUserBirthYear) <= 3
        ) {
          score += 100;
      ...
      const coAppearanceCount =
        coAppearanceScores.get(user._id.toString()) || 0;
